...
1、准备数据 x [None, 784]  y_true [None, 10]
2、卷积、激活、池化（两层）
结构：
卷积层一 卷积: 5*5*1，32个，strides=1 激活: tf.nn.relu 池化
        随机初始化权重, 偏置[32]
        对x进行形状的改变[None, 784]  [None, 28, 28, 1]
        [None, 28, 28, 1]-----> [None, 28, 28, 32]
        池化 2*2 ,strides2 [None, 28, 28, 32]---->[None, 14, 14, 32]
卷积层二 卷积: 5*5*32，64个filter，strides=1 激活: tf.nn.relu 池化：
        随机初始化权重,  权重：[5, 5, 32, 64]  偏置[64]
        卷积，激活，池化计算
        [None, 14, 14, 32]-----> [None, 14, 14, 64]
        池化 2*2, strides 2, [None, 14, 14, 64]---->[None, 7, 7, 64]
3、全连接层[None, 7, 7, 64]->[None, 7*7*64]*[7*7*64, 10]+ [10] =[None, 10]
4、求平均交叉熵损失
5、梯度下降求出损失
6、计算准确率
"""
